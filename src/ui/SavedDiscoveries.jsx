...
import { Bookmark, MapPin, Clock, Star, ExternalLink, Image, Crown, Palette, Music, ChefHat, Calendar } from "lucide-react";
  ...
  const DiscoveryCard = ({ discovery, index }) => {
    ...
    return (
      <motion.div
        ...
        className="bg-white rounded-3xl shadow-lg hover:shadow-2xl overflow-hidden transition-all duration-300"
      >
        <div className="flex">
          {/* Image */}
          ...
          <div className="flex-1 p-6">
            <div className="flex items-start justify-between mb-3">
              <h4 className="font-semibold text-gray-900 text-base truncate">{discovery.identified_item}</h4>
              <button className="text-gray-400 hover:text-gray-600">
                <ExternalLink className="w-5 h-5" />
              </button>
            </div>

            <p className="text-sm text-gray-700 mb-4 line-clamp-3">{discovery.description}</p>
  ...
  return (
    <div className="bg-white shadow-2xl rounded-3xl p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <span className="text-orange-600 text-base font-semibold">{displayDiscoveries.length} Saved</span>
      </div>
      ...
      <div className="space-y-6">
        {displayDiscoveries.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <h4 className="text-xl font-medium text-gray-500 mb-2">No Saved Discoveries</h4>
            <p className="text-gray-400 text-base">Use the AI scanner to discover and save cultural items</p>
          </div>
        )}
      </div>
    </div>
